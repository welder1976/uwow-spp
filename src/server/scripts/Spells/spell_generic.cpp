...
#include "ScriptedCreature.h"
  ...
  // };
	...
	/// Implementado por Pekado
class spell_forbbiden_zone : public AuraScript
{
	PrepareAuraScript(spell_forbbiden_zone);
	...
	void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
	{
		...
		{
			ChatHandler(player).PSendSysMessage("Zona no Permitida. Usted sera teletransportado para Pandarian!");
		case ALLIANCE:
		{
			player->TeleportTo(870, 880.48f, 295.72f, 503.12f, 0.66f); // Pandaria Alance Temple
			break;
		}
		case HORDE:
		{
			player->TeleportTo(870, 1606.59f, 921.75f, 470.62f, 3.30f); // Pandaria Horde Temple
			break;
		}
		default:
		{
			// if not have team  PANDAREN_NEUTRAL and TEAM_OTHER
			player->TeleportTo(870, 1342.597f, 491.379f, 457.901f, 2.34885f); // Pandaria
			break;
		}
		}
	...
	{
		AfterEffectApply += AuraEffectApplyFn(spell_forbbiden_zone::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
	}
};
...
void AddSC_generic_spell_scripts()
{
	...
	RegisterAuraScript(spell_forbbiden_zone);
